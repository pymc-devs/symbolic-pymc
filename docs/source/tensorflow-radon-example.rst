...
:numref:`python-setup` and :numref:`radon-data-download`, then we define the initial model
in :numref:`pymc4-radon-model`.
...
In :numref:`pymc4-radon-model-sample`, we estimate the model using the sample
...
:numref:`pymc4-sample-function`).  The same plots from the aforementioned notebook are
also reproduced here in `fig:pymc4-radon-plot-energy`_ and
`fig:pymc4-radon-plot-trace`_.

.. code-block:: python
    :caption: pymc4-sample-function
...
.. code-block:: python
    :caption: pymc4-radon-model-sample
    :name: pymc4-radon-model-sample

    az_trace = sample(model)

.. code-block:: python
    :caption: pymc4-radon-plot-setup
...
.. code-block:: python
    :caption: pymc4-radon-plot-energy
...
log-likelihood function generated by the model in :numref:`pymc4-radon-model`.
...
In :numref:`logp-func`, we use PyMC4's internal mechanisms to build the
...
:numref:`fgraph-specializations` creates and extracts these two objects.

.. code-block:: python
    :caption: fgraph-specializations
...
:numref:`tfp-normal-log-lik-graph` shows the graph produced by a normal
distribution in TFP.

.. code-block:: python
    :caption: tfp-normal-log-lik-graph
...
.. code-block:: python
    :caption: tfp-normal-log-lik-graph-print
...
:numref:`show-squared-diff-terms` shows the \ ``SquaredDifference``\
...
.. code-block:: python
    :caption: show-squared-diff-terms
...
:numref:`model-names-to-tfp-names` attempts to re-associate the TF and PyMC4 object names.

.. code-block:: python
    :caption: model-names-to-tfp-names
...
:numref:`grappler-normalize-function` provides a simple means of
applying \ ``grappler``\ .

.. code-block:: python
    :caption: grappler-normalize-function
...
In :numref:`grappler-normalize-function` we
run \ ``grappler``\  on the log-likelihood graph for a normal
random variable from :numref:`tfp-normal-log-lik-graph`.

.. code-block:: python
    :caption: grappler-normalize-test-graph
    :name: grappler-normalize-test-graph

    normal_log_lik_opt = normalize_tf_graph(normal_log_lik)

:numref:`opt-graph-output-cmp` compares the computed outputs for the original and
normalized graphs--given identical inputs.

.. code-block:: python
    :caption: opt-graph-output-cmp
...
From the output of :numref:`opt-graph-print`, we can see
...
In :numref:`kanren-shift-squaredo-func` and :numref:`tfp-normal-log-prob` we perform all
the necessary imports and create a few useful helper functions.

.. code-block:: python
    :caption: kanren-shift-squaredo-func
...
:numref:`tfp-normal-log-prob` is a function that will produce a meta graph for the
normalized form of a TFP normal log-likelihood.

In :numref:`shift-squared-subso`, we create the miniKanren goals that identify the
aforementioned normal log-likelihood "chains" and create the
re-centering/scaling substitutions.

.. code-block:: python
    :caption: shift-squared-subso
...
normal-normal hierarchical model in :numref:`non-trivial-transform-test-graph`.

.. code-block:: python
    :caption: non-trivial-transform-test-graph
...
:numref:`non-trivial-transform-test-graph-print` shows the form that
a graph representing a hierarchical normal-normal model will generally take
in TFP.

.. code-block:: python
    :caption: non-trivial-transform-test-graph-print
...
:numref:`non-trivial-transform-test-apply` runs our transformation and
:numref:`non-trivial-transform-test-print-graph` prints the resulting graph.

.. code-block:: python
    :caption: non-trivial-transform-test-apply
...
.. code-block:: python
    :caption: non-trivial-transform-test-print-graph
...
.. code-block:: python
    :caption: simplify-transformed-logpfn
...
:numref:`print-transformed-remaps` shows the replacements that were made
...
.. code-block:: python
    :caption: print-transformed-remaps
...
Likewise, :numref:`show-squared-diff-terms-in-trans` shows
\ ``SquaredDifference``\  subgraphs that appear in the
transformed log-likelihood.

.. code-block:: python
    :caption: show-squared-diff-terms-in-trans
...
:numref:`demo-diff-fgraph-output` shows the difference between a transformed and
non-transformed log-likelihood value given the same inputs.

Sampling from the new Log-likelihood
------------------------------------

In :numref:`sample-transformed-model`, we reproduce the remaining steps
...
.. code-block:: python
    :caption: sample-transformed-model
...
:numref:`kanren-shift-squaredo-func` could have been combined into a
...
:numref:`kanren-shift-squaredo-func` demonstrates **the way in which we were able to specify desired structure(s) within a graph**.
