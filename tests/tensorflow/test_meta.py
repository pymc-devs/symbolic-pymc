...
from symbolic_pymc.tensorflow.meta import (
    TFlowMetaTensor,
...
def test_meta_dtype_inference():
    ...
    add_mt = TFlowMetaOperator(mt.add.op_def, TFlowMetaNodeDef("Add", "my_add", {"T": 1}))
...
def test_meta_distributions():
    N = 100
    sigma_tf = tfd.Gamma(np.asarray(1.0), np.asarray(1.0)).sample()
    ...
    def _remove_obj(meta_obj):
        if hasattr(meta_obj, "_obj") and not isinstance(meta_obj, TFlowMetaOpDef):
            meta_obj._obj = None
...
def test_nodedef():
    ...
    y_test_new_mt = TFlowMetaOperator(y_test_mt.op.op_def, y_test_mt.op.node_def)(
        *y_test_mt.base_arguments
    )
